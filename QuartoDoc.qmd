...
agency_list <- create_agency_list(agencies)
agency_df <- filter(agencies, agency_name %in% agency_list)
# Elementary, municipal, group by agency minor type, rank order based on the agency_total_ext, rank order 
...
Each year, this taxing district sets a [**levy**]{style="color:purple;"} . This is the amount it needs to collect that year in order to provide services. (This was `r df %>% filter(district==districts[1]) %>% select(levy)` in 2021.)



Let's zoom in even further. If you are a [**property owner**]{style="color:red;"} in [**Riverside**]{style="color:purple;"} of a home worth [**`r df %>% filter(district==districts[1]) %>% select(av)`**]{style="color:red;"}, your taxable value (EAV) is [**`r df$eav[1]`**]{style="color:red;"}.
